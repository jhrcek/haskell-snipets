...
readChunks :: [Text] -> IO ()
readChunks cs = withServer $
    runSession myWdConfig . finallyClose $ do
...
chunkText maxChnkSize txt = reverse $ foldl chunk [] (T.words txt)
  where
    chunk [] word = [word]
    chunk (c : cs) word =
        if 1 + T.length word + T.length c <= maxChnkSize
...
selectVoice = do
  ...
  where
    clickItem str = clickElem (ByXPath $ T.concat ["//div/a/span/span[contains(.,'", str, "')]"])
