...
formatWordData Word{wWord = w, wPronUrls = prons, wDefinitions = defs} =
    w
        `append` if all null [prons, defs]
            then " NOT FOUND"
            else fmt "pron" prons `append` fmt "def" defs
  where
    fmt label items = concat $ zipWith (\item index -> concat ["\n  ", label, index, " = ", item]) items textIdxs
...
setupBeforeSearch = do
    ...
    waitPanelsLoaded
    waitWhile 10 $ do
        -- close all the panels except for the first one
